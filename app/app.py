...
class App:
    ...
    def pick_sender_receiver(self) -> None:
        send_btn = Button(self.frame, text='Send file', name='send_btn', width=24, height=4, command=self.show_connect_ui)
        send_btn.pack(side='left', expand=False, fill='x', padx=10)

        recv_btn = Button(self.frame, text='Receive file', name='recv_btn', width=24, height=4, command=self.show_host_ui)
    ...
    def show_host_ui(self) -> None:
        ...
        try:
            self.host_ip = self.connection_handler.get_host_ip()
            sock = self.connection_handler.host(self.host_ip, DEFAULT_PORT)
        except Exception as e:
            self.show_error(e)
            return

        conn_label = Label(self.frame, text='Enter sender ip', name='conn-label', fg=FONT_COLOR, bg=BACKGROUND_COLOR, font=10)
        ...
        confirm_conn_label = Button(ip_frame, text='Connect', command=lambda: self.accept_btn_click(ip.get(), sock))
    ...
    def show_connect_ui(self) -> None:
        ...
        conn_label = Label(self.frame, text='Connect to host', name='conn-label', fg=FONT_COLOR, bg=BACKGROUND_COLOR, font=10)
        conn_label.pack(pady=24)
        ...
        confirm_conn_label = Button(ip_frame, text='Connect', command=lambda: self.connect_btn_click(ip.get()))
        confirm_conn_label.grid(row=0, column=1)
        ip_frame.pack()
    ...
    def show_file_ui(self) -> None:
        ...
        if file:
            self.connection_handler.send_file(self.sock, file)

        done_label = Label(self.frame, text='File transfered succesfully', fg=FONT_COLOR, bg=BACKGROUND_COLOR, font=10)
    ...
    def show_transfer_ui(self) -> None:
        ...
        waiting_label = Label(self.frame, text='Waiting for file...', fg=FONT_COLOR, bg=BACKGROUND_COLOR, font=10)
        waiting_label.pack(expand=True, fill='x')

        self.connection_handler.recv_file(self.sock)
